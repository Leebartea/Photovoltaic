...
- Proposal Pricing now includes a **Local Cost Build-Up mode** alongside the benchmark $/Wp model, so installers in markets where local procurement prices differ significantly from international benchmarks (e.g., Nigeria) can enter per-unit local prices for panels, inverter, battery, BOS, logistics, permits, labour, and margin directly, with region-aware defaults and FX-safe arithmetic
